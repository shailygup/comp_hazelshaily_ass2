...
iptables -A tcpout -p tcp -m multiport --dport $TCPALLOW -j ACCEPT
iptables -A udpout -p tcp -m multiport --dport $UDPALLOW -j ACCEPT
iptables -A icmpout -p tcp -m multiport --dport $ICMPALLOW -j ACCEPT
